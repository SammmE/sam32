...
module ram2ddrxadc (
    ...
    // Generate ui_clk (100MHz -> 10ns period)
    initial begin
        ui_clk_o = 0;
    ...
    always @(posedge ui_clk_o) begin
        if (~ram_cen && ~ram_wen) begin
            logic [15:0] temp_word;
            
            if (mock_mem.exists(ram_a))
                temp_word = mock_mem[ram_a];
            else
                temp_word = 16'h0000;
                
            if (~ram_lb) temp_word[7:0]   = ram_dq_i[7:0];
            if (~ram_ub) temp_word[15:8]  = ram_dq_i[15:8];
            
            mock_mem[ram_a] = temp_word;
        end
    end
...
module ram_tb();
    ...
    task automatic write_mem(input logic [31:0] addr_in, input logic [31:0] data_in_val, input logic [1:0] w_in, input string desc_in);
        ...
        if (w_in == 2'b10) begin // Word
            shadow_mem[addr_in] = data_in_val;
        end else if (w_in == 2'b01) begin // hword
    ...
    task automatic read_mem(input logic [31:0] addr_in, input logic [1:0] w_in, input logic lu_in, input string desc_in);
        address = addr_in;
        width = w_in;
        load_unsigned = lu_in;
        store = 0;
        ...
        load = 0;
        @(posedge ui_clk); 
        $display("Read passed for %s (Addr: %h, Data: %h, Width: %b, Unsigned: %b)", desc_in, addr_in, data_out, w_in, lu_in);
    endtask
    ...
    initial begin
        ...
        rst = 1;
        address = 0; data_in = 0; store = 0; load = 0; width = 0; load_unsigned = 0;
        #100;
        rst = 0;
        ...
        read_mem(32'h0000_1000, 2'b10, 1'b1, "Word Read");
        ...
        read_mem(32'h0000_3000, 2'b00, 1'b1, "Byte Read (Even, Unsigned)");
        read_mem(32'h0000_3000, 2'b00, 1'b0, "Byte Read (Even, Signed)");
        
        // Byte Write & Read (Odd Address)
        write_mem(32'h0000_3001, 32'h0000_00CD, 2'b00, "Byte Write (Odd)");
        read_mem(32'h0000_3001, 2'b00, 1'b1, "Byte Read (Odd, Unsigned)");
        read_mem(32'h0000_3001, 2'b00, 1'b0, "Byte Read (Odd, Signed)");
        ...
        read_mem(32'h0000_4000, 2'b10, 1'b1, "Word Read 1");
        read_mem(32'h0000_4004, 2'b10, 1'b1, "Word Read 2");
        ...
        for (i = 0; i < 50; i = i + 1) begin
            ...
            rand_data = $urandom;
            rand_lu = $urandom_range(0, 1);
            
            desc = "Random Write";
            write_mem(rand_addr, rand_data, rand_width, desc);
            
            desc = "Random Read";
            read_mem(rand_addr, rand_width, rand_lu, desc);
        end
